...
def get_sms_text(doc):
	...
	emailed = frappe.db.get_value("Clinic Settings", None, "sms_emailed")
	sms_text['emailed'] = frappe.render_template(emailed, context)
	printed = frappe.db.get_value("Clinic Settings", None, "sms_printed")
	sms_text['printed'] = frappe.render_template(printed, context)
	return sms_text

def send_registration_sms(doc):
	if (frappe.db.get_value("Clinic Settings", None, "reg_sms")=='1'):
		if doc.mobile:
			context = {"doc": doc, "alert": doc, "comments": None}
			if doc.get("_comments"):
				context["comments"] = json.loads(doc.get("_comments"))
			messages = frappe.db.get_value("Clinic Settings", None, "reg_msg")
...
def get_receivable_account(company):
	receivable_account = get_account(None, "receivable_account", "Clinic Settings", company)
...
def get_income_account(physician, company):
	...
	income_account = get_account(None, "income_account", "Clinic Settings", company)
	if income_account:
		return income_account
